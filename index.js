...
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://pctlqnityusjxlicreax.supabase.co';
...
async function rpc(method, params = []) {
    try {
        ...
        const res = await axios.post(RPC_URL, {
            jsonrpc: '1.0', id: 'explorer', method, params
        }, {
...
app.get('/api/info', async (req, res) => {
    ...
    if (info.success) {
        res.json({...info.result, connections: net.result?.connections || 0, online: true});
    } else {
        res.json({blocks: '—', online: false, error: 'Node offline'});
    }
});
...
app.get('/api/node/status', async (req, res) => {
    ...
    if (info.success) {
        res.json({
            ...
            connections: net.result?.connections || 0,
            difficulty: info.result.difficulty,
            chain: info.result.chain
        });
    } else {
        res.json({ online: false, error: info.error });
    }
});
...
app.post('/api/wallet/new-address', async (req, res) => {
    ...
    const { data: existing } = await supabase
        .from('hon_wallets').select('address').eq('user_id', user_id).single();
    if (existing) return res.json({ address: existing.address });

    // Generate new
    await rpc('loadwallet', ['honwallet']).catch(() => {});
    const addr = await rpc('getnewaddress', ['', 'bech32']);
    if (!addr.success) return res.status(500).json({ error: 'Failed' });
...
app.post('/api/mining/share', async (req, res) => {
    ...
    if (result.success) {
        const { data: earnings } = await supabase
            .from('hon_earnings').select('*').eq('user_id', user_id).single();
        const newAmount = (earnings?.amount || 0) + 2000;
        const newBlocks = (earnings?.blocks_mined || 0) + 1;
        await supabase.from('hon_earnings').upsert({
            user_id, amount: newAmount, blocks_mined: newBlocks,
...
app.get('/api/mining/stats/:user_id', async (req, res) => {
    const { data } = await supabase
        .from('hon_earnings').select('*').eq('user_id', req.params.user_id).single();
...
app.get('/', (req, res) => {
    res.json({ status: 'SHIVRAI HON API', version: '2.0.0' });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 SHIVRAI HON API running on port ${PORT}`));
